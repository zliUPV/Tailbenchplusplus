...
# LAUNCH THE SERVER
TBENCH_MAXREQS=${MAXREQS} TBENCH_WARMUPREQS=${WARMUPREQS} TBENCH_NCLIENTS=${N_CLIENTS} taskset -c 5,29 ${DIR}/out-perf.masstree/benchmarks/dbtest_server_networked --verbose --bench tpcc --num-threads ${NUM_THREADS} --scale-factor ${NUM_WAREHOUSES} --retry-aborted-transactions --ops-per-worker 10000000 &
...
for ((i=0; i<${N_CLIENTS}; i++)); do
    ...
    # LAUNCH THE CLIENT
    TBENCH_QPS=${QPS} TBENCH_MINSLEEPNS=100 TBENCH_CLIENT_THREADS=${TBENCH_CLIENT_THREADS} TBENCH_ID=${i} taskset -c ${cpu_set} ${DIR}/out-perf.masstree/benchmarks/dbtest_client_networked 2>&1 | tee client_${i}.sal &
	echo $! > client_${i}.pid
