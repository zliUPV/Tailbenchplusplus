...
# LAUNCH THE SERVER
TBENCH_SERVER_PORT=${PORT} TBENCH_MAXREQS=${MAXREQS} TBENCH_WARMUPREQS=${WARMUPREQS} TBENCH_NCLIENTS=${N_CLIENTS} \
taskset -c 5,29 \
${BINDIR}/moses_server_networked -config ./moses.ini -input-file ${DATA_ROOT}/moses/testTerms -threads ${SERVER_THREADS} -num-tasks 1000000 -verbose 0 &
...
for ((i=0; i<${N_CLIENTS}; i++)); do
    ...
    taskset -c $cpu_set \
    ${BINDIR}/moses_client_networked 2>&1 \
    | tee client_${i}.sal &
