...
# Server
taskset -c 6,30 ${JDK_PATH}/bin/java -Djava.library.path=. -XX:ParallelGCThreads=1 -XX:+UseSerialGC -XX:NewRatio=1 -XX:NewSize=7000m -Xloggc:gc.log -Xms10000m -Xmx10000m -Xrs spec.jbb.JBBmain -propfile SPECjbb_mt.props &
...
for ((i=0; i<${N_CLIENTS}; i++)); do
    export TBENCH_ID=${i}
    taskset -c ${cores_taskset} ./client &
    echo $! > client_${i}.pid
done
